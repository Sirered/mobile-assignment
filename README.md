...
I did the same steps as outlined in Tutorial 2 (create a Procfile with info regarding my Django project, as well as a Dockerfile and github workflows that defines the job 'Deployment' and what it should do, and set the secrets that are used by the Deployment) and push those changes into a main branch which will allow them to be deployed to the PBP Paas. As of now there are some issues with the deployed app, so I have done this assignment using localhost rather than my deployed app

**Create a login page in the Flutter project.**
Firstly, in my Django Project I install an app called corsheaders and setup the settings.py to recognise that as an installed app, set it's associated middleware in the Middleware list in settings.py and add all necessary constants for corsheaders to work as we want it to in the settings.py. Then I created an authentication app using `python manage.py startapp authentication` in which I defined a login function and a logout function. They work the same as the login and logout from previous assignment, it's just that it reads the request differently (if in the original we have request.POST.get('data'), we would have to do request.POST\['data'\]) and instead of returning an HttpResponseRedirect, we return a JSONResponse with all of the information that our flutter app needs to read (like if it was a success, message and the username of the user, if login was successful). Afterwards in the authentication application I make a urls.py, make routing paths in the urls.py to the functions in the views.py of the authentication app, and added a path that includes the authentication urls.py in the project directory

Afterwards, in a new file called login.dart I created a stateful wisget called LoginPage, with an instance of _LoginPageState to be assigned as it's state. I also define the _LoginPageState widget which has final (meaning unchanging) variables of type TextEditingController that will be used to track the contents of the TextFields that the user will type in. They are named _usernameController and _passwordController. There is also a request variable within the build function that is of the class CookieRequest witch tracks cookies and allows us to send http requests to our Django project. Then we override the Build method within _LoginPageState, so that on build it will create a Scaffold with an Appbar, and it's body (which is the main body we will see on the webpage) is a Container (used for padding purposes) with a child of the Column class (for positioning purposes and to store a list of children widgets that will be placed below one another according to the order of the list that they're in). The Column class has the following children:
* A TextField with the _usernameController as it's controller and InputDecoration that states that this field is for "Username"
* A TextField with the _passwordController as it's controller and InputDecoration that states that this field is for "Password"
* An ElevatedButton, with the Text "Login"that when clicked will take the contents of the textfields, by getting the text attribute of the controllers, await a request to the Django project, using the request CookieRequest, that should be routed to the login function in the authentication app, which will attempt to login to the application. If it succeeds some session data will be stored in the request CookieRequest, the user will be Navigator.pushReplacemented to the HomePage of the app, and will be given a success message in a snackbar (the message was sent from the Django login). Otherwise it will display an AlertDialog stating the login failed as well as why it failed (which is data sent from the Django project)
* An ElevatedButton, with the text 'Register' that Navigator.pushes the user to the register page (for the bonus)

I then set the home of MyApp to login and wrapped MaterialApp that used to be what was returned by the build function, with a Provider app that creates the requst CookieRequest and will allow child widgets of the app to utilise that request CookieRequest.

**Integrate the Django authentication system with the Flutter project.**

Most of the details/steps are already listed in the last section prior.

**Create a custom model according to your Django application project.**

I ran the show_json function in my Django Project that serializes all items stored in the database and serialises it to json. I then copied the output to QuickType, set the language to Dart, source type to JSON and name as Item. I created a new models folder in lib, in which I created an item.dart file. I then copied the code from QuickType and pasted the code into item.dart. That code now defines the Item class, it's attributes/fields and how to convert it from JSON to an instance of an Item class. I also added code in the condtructor that will assign a color to the cardColor variable, according to the color taken from the Django model and it will use that cardColor for when we build the cards that will represent the items. I also added textColor, which by default is black, but is white when the cardColor is too dark.

**Create a page containing a list of all items available at the JSON endpoint in Django that you have deployed.**

In a new file called view_screen.dart (it was actually created last week for a bonus, but all of the contents from last week have since been removed) I define a StatefulWidget named ItemPage, with an instance of _ItemPageState to be assigned as it's state. I then define the ItemPageState with a Future List of Items that is updated by the result of an asynchronous function called fetchData. This FetchData uses the request CookieRequest to send a request to the Django Project, which should be routed to the show_json function and takes the Json from the response. Then for each item within the json, it uses the serialisation logic from the Item model to convert the json to an object of type Item, then addsthat item to a list. The fetchData function returns that list.

The _ItemPageState widget also overrides the build function, with the normal return Scafold with appbar and drawer, however for the body of the Scaffold it has a FutureBuilder. This FutureBuilder, which will be used to build the current state has fetchData as it's future, so snapshots of data will be taken from the result of the fetchData function, and a builder that will be using those snapshots to build the current state. If snapshot.data is null (meaning that nothing was successfully returned by fetchData yet) a circular progress indicator will be shown. If !snapshot.hasData (so if the data was successfully returned, but that data is empty), it will display data stating that "No item data available". Otherwise it will return a Gridview.builder that will build a card for every item and put them in a grid. It has an itemCount equal to the amount of data found in the snapshot (so it knows to build that many children), a gridDelegate that delegates how much space is used by each child (basically defining the properties of the grid) and an itemBuilder that for each item will build a Card, with color defined by the carcColor of the item (this is done by getting snapshot.data!\[index\].fields.cardColor. All references of fields from the Item class within this builder will be using a similar format, just changing 'cardColor' to the appropriate attribute name), and a Padding widget as it's child. This Padding widget has EdgeInsets(20.0) as it's padding and a child of the ListView widget class. The Listview has the following children in this order:
* A Text that displays the name of the item, with bold font, size of 23 and the textColor of the item
* A SizedBox of height 10 for vertical spacing
* A Text that displays the amount of the item, with bold font, size of 15 and the textColor of the item
* A SizedBox of height 10 for vertical spacing
* A Text that displays the text "Description:" with bold font size of 15 vand the textColor of the item
* A Text that displays the description of the item, with normal font, size of 15 and the textColor of the item

**Create a detail page for each item listed on the Item list page.**

In a new file called detail_item.dart I define a stateless widget named DetailPage with a final item of the Item class as one of it's variables. It also overrides the build function that returns a Scaffold with an appBar and a body of the Center Widget class (do the data will be in the center), with a child of the Card class. The Card is of the color defined by cardColor of the item (using item.fields.cardColor. All other references of item attributes within this Page will use a similar format), rounded border radius, EdgeInsets.all(20.0) for the margin and a child of the Container class. The Container determines the dimensions of the card (width and height of 400), padding of EdgeInsets.all(16.0) and a child of the Column class. The Column class has CrossAxisAlignment set to start (so all of the text will be on the left side of the card) and has the following children in this order:
* A Text containing the name of the item, with bold font, size of 23 and the textColor of the item
* A SizedBox of height 10 for vertical spacing
* For all attributes of the item class other than pk, model, name, color, cardColor, textColor and description, it will have a Text displaying the attribute, with bold font, size of 15 and the textColor of the item, followed by a SizedBox of height 10 for vertical spacing
* A Text that displays the text "Description:" with bold font size of 15 vand the textColor of the item
* A Text that displays the description of the item, with normal font, size of 15 and the textColor of the item
* An ElevatedButton that on press will call Navigator.pop() which will return the user to the Item list page

Then in view_screen.dart I wrap the Padding that was a child of the Card(s) that are built by the Gridview.builder, with an Inkwell that onTap will Navigator.push() the DetailPage, passing snapshot.data!\[index\] as the item that will be used to build the DetailPage
